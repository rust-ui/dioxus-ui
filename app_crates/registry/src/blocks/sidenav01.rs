            ...
            div { class: "mx-auto grid max-w-7xl gap-6 px-6 py-8 lg:grid-cols-[18rem_minmax(0,1fr)]",
                aside { class: "flex flex-col rounded-xl border bg-sidenav p-3 text-sidenav-foreground shadow-sm",
                    div { class: "space-y-2 border-b border-sidenav-border/70 pb-3",
                        div { class: "text-lg font-semibold", "Grouped Sidenav" }
                        p { class: "text-xs text-sidenav-foreground/70", "A structured sidebar with grouped links and a simple content panel." }
                        div { class: "relative",
                            Input { r#type: InputType::Search, placeholder: "Search the docs...", class: "h-8 pl-9 bg-background" }
                        ...
                        section { class: "space-y-2",
                            ...
                            div { class: "space-y-1",
                                for (href, label) in COMPONENT_LINKS {
                                    a { href: *href, class: "flex items-center gap-2 rounded-md px-2 py-2 text-sm hover:bg-sidenav-accent hover:text-sidenav-accent-foreground",
                    ...
                    div { class: "mt-auto border-t border-sidenav-border/70 pt-3 text-xs text-sidenav-foreground/70", "Signed in as Rustify" }
                }
                main { class: "rounded-xl border bg-background p-6 shadow-sm",
                    div { class: "space-y-4",
                        ...
                        h1 { class: "text-3xl font-semibold tracking-tight", "Grouped Sidenav" }
                        p { class: "max-w-prose text-sm text-muted-foreground", "A structured sidebar with grouped links and a simple content panel." }
                        div { class: "grid gap-4 md:grid-cols-2",
                            div { class: "rounded-lg border bg-card p-4",
                                div { class: "text-sm font-medium", "Navigation" }
                                p { class: "mt-2 text-sm text-muted-foreground", "This block shows the shell layout, sidebar links, and content framing used by the Leptos demo." }
                            }
                            div { class: "rounded-lg border bg-card p-4",
                                div { class: "text-sm font-medium", "Actions" }
                                div { class: "mt-3 flex flex-wrap gap-2",
                                    Button { variant: ButtonVariant::Outline, size: ButtonSize::Sm, "Copy link" }
                                    Button { variant: ButtonVariant::Ghost, size: ButtonSize::Sm, "Open docs" }
                                }
                            }
                        }
                    }
